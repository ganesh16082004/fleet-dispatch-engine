    ...
    <main className="shell">
      ...
      <section className="workspace">
        ...
        <aside className="right-column">
          <article className="card panel driver-panel"><div className="card-head"><div><div className="section-kicker">FLEET CONTROL</div><h2>Driver operations</h2><div className="card-hint">Heartbeat · node · assignment state</div></div><span className="counter">{driverList.length}</span></div><div className="table-wrap"><table className="table"><thead><tr><th>Driver</th><th>Status</th><th>Node</th><th>Pulse</th></tr></thead><tbody>{driverList.length === 0 ? <tr><td colSpan={4} className="empty">No drivers registered</td></tr> : driverList.slice(0, 12).map((driver) => { const live = locations[driver.id]; return <tr key={driver.id}><td className="strong">#{driver.id}</td><td><span className={`status ${statusClass(driver.status)}`}>{driver.status}</span></td><td>{live?.nodeId ?? driver.currentNode}</td><td className="muted">{live ? timeLabel(live.timestampMillis) : "—"}</td></tr>; })}</tbody></table></div></article>
